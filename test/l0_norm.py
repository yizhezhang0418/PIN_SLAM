...
class denoise_l0norm:
    ...
    def estimate_normals(self, k = 20):
        ...
        # # 可选：根据视点位置调整法线方向
        # self.pc_load.orient_normals_consistent_tangent_plane(k)
        ...
        while np.max(np.abs(diff_op - sita)) > threshold:
            ...
            for i in range(point_num):
                neighbor_indices = np.array(neighbors_normal_index[i])  # 获取邻居索引
    ...
    for filename in os.listdir(directory_path):
        ...
        if os.path.isfile(file_path):
            with open(file_path, 'r') as file:
                ...
                ply_file_path = os.path.join(args.output_folder, filename)
